...
TEMP_DIR=${ORIGINAL_FILE%/*}'/_floatsignTemp'
# options start index
...
function bundle_id_for_provison {
    ...
    local FULL_BUNDLE_ID=$(PlistBuddy -c 'Print :Entitlements:application-identifier' /dev/stdin <<< "$(sudo security cms -D -i "$1")")
    checkStatus
    echo "${FULL_BUNDLE_ID#*.}"
}
...
function resign {
    ...
    sudo security cms -D -i "$NEW_PROVISION" > "$TEMP_DIR/profile.plist"
    checkStatus
    ...
    then
        ...
        do
            ...
            then
                ...
                /usr/bin/codesign --keychain /Users/administrator/Library/Keychains/login.keychain --force --sign "$CERTIFICATE" "$framework"
#                /usr/bin/codesign ${VERBOSE} ${KEYCHAIN_FLAG} -f -s "$CERTIFICATE" "$framework"
    ...
    then
        ...
        sudo security cms -D -i "$TEMP_DIR/old-embedded.mobileprovision" > "$TEMP_DIR/old-embedded-profile.plist"
        ...
        # shellcheck disable=SC2086
        security unlock-keychain -p asd123 "/Users/administrator/Library/Keychains/login.keychain"
/usr/bin/codesign --keychain /Users/administrator/Library/Keychains/login.keychain --force --sign "$CERTIFICATE" --entitlements "$TEMP_DIR/newEntitlements" "$APP_PATH"
