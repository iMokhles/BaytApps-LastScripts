...
/Library/Server/Web/Data/Sites/cloud.baytapps.me/files_store/accounts/scripts_shs/icon_generator.sh "$TEMP_PATH/appIcon.png" "$ICONS_TEMP_PATH"
 ...
 #  	security unlock-keychain -p TotoaTEAM2016 "/Users/TotoaTEAM/Library/Keychains/login.keychain"
	...
	if [ "$dupNUM" -eq "$DUPLINumber" ];then
		echo "URL_TO_INSTALL_APP=itms-services://?action=download-manifest&url=$SERVERURL/Plist/$HOOKED_APP_BUNDLE_ID-$dupNUM.plist=LAST_PART_INSTALL_LINK"
        #mv "$AppIPA" "$AppFolder/$HOOKED_APP_BUNDLE_ID-$dupNUM.ipa"
#cp "$AppFolder/$HOOKED_APP_BUNDLE_ID-$dupNUM.ipa" "$OUTPUT_PATH/ipa"
#rm -rf "$AppFolder/$HOOKED_APP_BUNDLE_ID-$dupNUM.ipa" || true
rm $AppIPA
	fi
done
