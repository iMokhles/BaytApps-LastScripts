...
cp "/Library/Server/Web/Data/Sites/Default/files_store/scripts_shs/appIcon.png" "$TEMP_PATH/appIcon.png"
...
/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName $HOOKED_APP_NAME$dupNUM" "$APP_PATH/Info.plist"
/usr/libexec/PlistBuddy -c "Set :CFBundleName $HOOKED_APP_NAME$dupNUM" "$APP_PATH/Info.plist"
...
echo "$ServerDownURL/${7}ipa/$HOOKED_APP_BUNDLE_ID-$dupNUM.ipa"
...
/usr/libexec/PlistBuddy -c "Set :items:0:assets:0:url $ServerDownURL/${7}ipa/$HOOKED_APP_BUNDLE_ID-$dupNUM.ipa" $OTA_MODIFIED_PLIST_PATH
...
/usr/libexec/PlistBuddy -c "Set :items:0:metadata:title $HOOKED_APP_NAME$dupNUM" $OTA_MODIFIED_PLIST_PATH
